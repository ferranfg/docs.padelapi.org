...
So every match in the API carries the **Padel API Watchability Index**: an integer from `0` to `100` that answers one question, *how much is this match worth watching*, before a single point is played.
...
<Steps>
  <Step title="How good the players are" icon="chart-line">
    The [Elo rating](/guides/padel-elo-rating) of the players on court, read against the range the professional tour actually occupies, and leaning towards the stronger of the two pairs: seeing one of the best pairs on tour is a draw in itself. This is the heaviest ingredient: a final between two mid-table pairs is not the same event as a final between the top two.
  </Step>
  <Step title="How close it should be" icon="scale-balanced">
    The [calibrated win probability](/guides/padel-match-prediction) for the matchup. A 50/50 contributes everything it can, a 95/5 contributes almost nothing. It is what separates a genuine contest from a formality between the same names.
  </Step>
  <Step title="What is at stake" icon="trophy">
    The round. A final counts for more than a semi-final, which counts for more than a quarter-final, down to the opening rounds. Enough to break ties between comparable matches, not enough to push a dull final above a great quarter-final. This ingredient only counts inside the main draw: in any other draw it contributes nothing, so those matches score below a comparable main draw match.
  </Step>
</Steps>
...
- **New pairs.** Ratings are individual, so a pair playing together for the first time reads the same as one with two seasons of joint titles.
...
<AccordionGroup>
  ...
  <Accordion title="Why does a top pair against a weaker side still rate reasonably?">
    Because level and closeness are weighed against each other, rather than one cancelling the other out. A one sided fixture gives up most of what the closeness component has to offer, but having one of the best pairs on tour on court is worth something on its own, and the index says so. It will still sit clearly below an even match between comparable pairs in the same round.
  </Accordion>
