...
| Field | What it means |
|---|---|
| `tie.id` | Identifies the tie. The three rubbers carry the same one |
...
With `?draw=all&sort_by=round`, the main draw is ordered first and the other draws follow. `tie.rubber` is the last tiebreaker of the sort, so it only orders matches that are otherwise equal: it does not group a tie, and the three rubbers do not come back next to each other. To rebuild a tie, group by `tie.id`.

The tournament matches endpoint also filters on the parts of a tie:

```bash
# The three rubbers of one tie
curl 'https://padelapi.org/api/tournaments/258/matches?draw=all&tie[id]=9f2a7c41e0d3' \
  -H 'Authorization: Bearer YOUR_API_TOKEN'
...
| `seeds` | match | No seeding is published for team tournaments, so both sides come back `null` |
...
- **Do not compute ratings or statistics from rubbers.** A pairing put together for a team lasts one week, so the API leaves team events out of the [Elo](/guides/padel-elo-rating), and player and pair [statistics](/guides/padel-statistics) are computed over main draw matches as everywhere else. Recomputing them locally reintroduces exactly what was excluded on purpose.
...
<AccordionGroup>
  ...
  <Accordion title="Why are the seeds null?">
    No seeding information is published for team tournaments, so both sides of every match come back `null`.
  </Accordion>
