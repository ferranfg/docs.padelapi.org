...
| `tie.team_1`, `tie.team_2` | The two teams meeting each other. National teams are identified by their ISO 3166-1 alpha-2 code |
...
<Warning>
  Read the team from `tie`, never from the `country` of the players. The team a pair represents is what decides the tie, and it is not always the same as the country on a player's profile.
</Warning>
...
{
  "draw": "group",
  "tie": { "group": "A", "team_1": "AR", "team_2": "IT", "rubber": 3 },
  "status": "not_played",
  "score": null,
  "winner": null,
  "...": "..."
...
|---|---|---|---|
| 1 | Stupaczuk/Di Nenno – Coello/Nieto | `team_2` | ES leads 1–0 |
| 2 | Tapia/Chingotto – Lebron/Galan | `team_1` | 1–1 |
| 3 | Augsburger/Libaak – Yanguas/Navarro | `team_1` | **AR wins 2–1** |
...
With `?draw=all&sort_by=round`, the main draw is ordered first and the other draws follow. `tie.rubber` is the last tiebreaker of the sort, so the three matches of a tie, which share a date and a round, come back as rubber 1, 2 and 3.
...
curl 'https://padelapi.org/api/tournaments/258/matches?draw=all&tie[team_1]=AR' \
  -H 'Authorization: Bearer YOUR_API_TOKEN'
```

`tie[team_1]` and `tie[team_2]` match one side of the tie, so a team's full campaign is the union of both. Identifiers are matched uppercase.
