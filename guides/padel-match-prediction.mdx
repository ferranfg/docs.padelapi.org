...
Most padel predictions you find online are opinions. This one is a number you can audit: a **win probability produced by the Padel API Elo model**, calibrated so that when it says 78%, the favourite really wins about 78% of the time.
...
<AccordionGroup>
  ...
  <Accordion title="How often do the ratings change?">
    Every completed match moves the ratings, and new results are applied daily. A simulation run the day after a match already includes its result.
  </Accordion>
</AccordionGroup>
