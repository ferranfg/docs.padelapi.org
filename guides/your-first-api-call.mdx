...
Let's fetch the list of available seasons with [`GET /api/seasons`](/api-reference/season/list-seasons). This is a simple endpoint that returns all padel seasons available in the API.
...
Now let's try something more interesting. Fetching the current player rankings via [`GET /api/players`](/api-reference/player/list-players):
