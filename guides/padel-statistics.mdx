...
The most granular level. Every match with available data includes stats for both teams, broken down by set. Endpoint: [`GET /api/matches/{match_id}/stats`](/api-reference/match/show-match-stats).
...
Career-level aggregates for any player. Useful for comparing trajectories, building rankings models, or analyzing form over specific periods. Endpoint: [`GET /api/players/{player_id}/stats`](/api-reference/player/show-player-stats).

<Note>
  Player and pair statistics are computed over **main draw matches only**. Qualification rounds, and any other draw, are excluded from every metric, so a win percentage is not diluted by the matches a pair played to enter the tournament. The [Elo rating](/guides/padel-elo-rating) works the other way: it counts every settled match of a regular tournament, qualification included, because a result against a strong opponent is informative wherever it happens. To read the other draws themselves, use the `draw` filter on the [match listings](/api-reference/match/list-matches): `?draw=qualy` for the qualification draw alone, or `?draw=all` for every draw in a single list.
</Note>
...
Padel is a doubles sport, and individual numbers only tell part of the story. Pair stats measure how two specific players perform **together**. Endpoint: [`GET /api/pairs/{player_1_id}-{player_2_id}/stats`](/api-reference/pair/show-pair-stats).
...
<CardGroup cols={3}>
  <Card title="Player Stats Endpoint" icon="user" href="/api-reference/player/show-player-stats">
    Full reference for career win rate, titles and progression
  </Card>
  <Card title="Match Stats Endpoint" icon="chart-bar" href="/api-reference/match/show-match-stats">
    Serve %, break points and streaks, set by set
  </Card>
  <Card title="Pair Stats Endpoint" icon="people-group" href="/api-reference/pair/show-pair-stats">
