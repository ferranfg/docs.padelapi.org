...
<Note>
  Player and pair statistics are computed over **main draw matches of pairs tournaments only**. Qualification rounds, and any other draw, are excluded from every metric, so a win percentage is not diluted by the matches a pair played to enter the tournament. [Team tournaments](/guides/team-tournaments) are left out entirely, as they are from the Elo rating.
</Note>
