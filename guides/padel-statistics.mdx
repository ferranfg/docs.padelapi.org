...
<Note>
  Player and pair statistics are computed over **main draw matches only**. Qualification rounds, and any other draw, are excluded from every metric, so a win percentage is not diluted by the matches a pair played to enter the tournament.
</Note>
