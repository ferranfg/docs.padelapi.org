...
Tournaments progress from `pending` through `finished`, unless cancelled or postponed. See the [Data Synchronization](/guides/data-synchronization#tournaments) guide for best practices on keeping your data up to date.
...
| `postponed` | Tournament was postponed; dates are the last announced |
...
| `not_played` | Both pairs were known and the match never took place, with no winner awarded |
...
<AccordionGroup>
  ...
  <Accordion title="What is the difference between retired, walkover, bye and not_played?">
    A `retired` match started and has a partial score — one team couldn't continue. A `walkover` was awarded before play began, typically due to an injury or withdrawal, and has a winner. A `bye` means no match took place because there was no opponent to play. A `not_played` match had both pairs on the sheet and never took place, so there is no winner.
  </Accordion>
  <Accordion title="Which statuses are permanent?">
    Some match statuses are permanent once set and will never change: `finished`, `retired` and `walkover`. Others are variable and will change as the match progresses: `scheduled`, `live`, and `ended`. `bye` and `not_played` are stable in practice, but they describe a match that was not played rather than a result, so they yield if a result is later recorded for that match.
  </Accordion>
</AccordionGroup>
...
<Tip>
  Use `coverage: "full"` matches for statistical analysis. Note that `full` guarantees game-level completeness; individual points within a game may occasionally be missing.
</Tip>
