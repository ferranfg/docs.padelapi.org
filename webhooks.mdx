...
Create a webhook subscription by sending the URL you control and the list of events you want to receive:
...
Change the URL or the list of subscribed events without recreating the webhook (and without losing the secret):
...
To stop a webhook permanently, delete it:
...
<AccordionGroup>
  ...
  <Accordion title="Handling burst traffic during live tournaments">
    During a busy tournament day you can receive hundreds of `match.updated` events per minute. Always queue work in a background job and return `200` immediately — synchronous processing inside the webhook handler will hit the 3-second timeout and trigger needless retries.
  </Accordion>
