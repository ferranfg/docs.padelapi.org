...
description: "Complete reference for Padel API HTTP response codes. What every success, redirect and error code means, and how to handle 401, 402, 404, 422, 429, and 500 in your application."
---
...
The requested resource has been moved to a new location. This typically happens when a player, tournament, or match record has been updated or merged with new data.
...
Your current subscription plan does not include access to this resource.
...
You have exceeded the API rate limit. There are two, requests per minute and requests per day, and both return this same code.
...
2. If the 429s keep coming after a couple of minutes at a low rate, you are against the daily limit rather than the per-minute one, and waiting a minute will not clear it. The [Logs & Quota](https://padelapi.org/explore/resources/logs) page shows which of the two you have hit
3. Monitor the `X-RateLimit-Remaining` header to avoid hitting the limit
