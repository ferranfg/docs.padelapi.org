...
| `coverage` | Point-by-point data completeness: `tracking` (match live, collecting data), `full` (all games tracked), `partial` (some games missing due to source feed gaps), or `null` (no data available) |
...
The WebSocket event payload has the same structure as the `/api/matches/{match}/live` REST response — `id`, `self`, `status`, `coverage`, `channel`, `sets`, and `connections`.
