...
When a match ends, the `status` field changes to `ended`. See [Match Statuses](/statuses#match-statuses) for details. Use this to clean up your subscription:
...
channel.bind('App\\PadelApi\\Events\\MatchLiveUpdated', (data) => {
  ...
  if (data.status === 'ended') {
    ...
    console.log('Match ended, unsubscribed');
  }
});
```
...
def on_event(data):
    ...
    if match['status'] == 'ended':
        pusher.unsubscribe('matches.7243')
        print('Match ended, unsubscribed')
```
...
4. **Detect match end** — When `status` changes to `ended`, unsubscribe from the channel
