---
title: "WebSockets"
description: "Build real-time padel scoreboards and live trackers with the Padel API: REST polling, WebSocket subscriptions, and working examples."
---
...
<Note>
  Looking for *resource changes* rather than point-by-point data? Use [Webhooks](/webhooks) instead — they push notifications when a tournament, match, or player is created, updated, or deleted, without keeping a connection open.
</Note>
...
First, discover which matches are currently being played using [`GET /api/live`](/api-reference/live/list-live-matches):
...
Once you have a match ID, fetch its point-by-point data with [`GET /api/matches/{id}/live`](/api-reference/match/show-match-point-by-point):
...
```json
{
  ...
  "status": "live",
  "coverage": "full",
  "channel": "matches.9f2a1c8e4b6d0a53",
  "sets": [
    {
      "set_number": 1,
      "set_score": "6-2",
...
| `channel` | Pusher channel name for real-time updates. Issued per match, so read it from the response |
...
| **Cluster** | `eu` |
| **Channel name** | The `channel` field returned by the live endpoints |
| **Event name** | `App\PadelApi\Events\MatchLiveUpdated` |

<Note>
  No authentication handshake is required to subscribe. You need the Pusher app key, the cluster and the `channel` name returned by the live endpoints — channel names are issued per match and cannot be built from the match id.
</Note>
...
const channel = pusher.subscribe('matches.9f2a1c8e4b6d0a53');
...
channel = pusher.subscribe('matches.9f2a1c8e4b6d0a53')
...
channel.bind('App\\PadelApi\\Events\\MatchLiveUpdated', (data) => {
  ...
  if (data.status === 'finished') {
    pusher.unsubscribe('matches.9f2a1c8e4b6d0a53');
...
1. **Find live matches** — [`GET /api/live`](/api-reference/live/list-live-matches)
2. **Get initial state** — [`GET /api/matches/{id}/live`](/api-reference/match/show-match-point-by-point)
3. **Subscribe to updates** — Connect to the Pusher channel named in the `channel` field
...
<AccordionGroup>
  <Accordion title="REST polling vs WebSockets vs Webhooks — when to use which">
    Use **REST polling** when you need a simple integration, are building server-side batch processes, or your environment doesn't support persistent connections. Use **WebSockets** for user-facing applications where latency matters — live scoreboards, mobile apps, or real-time dashboards. Use [**Webhooks**](/webhooks) for backend integrations that react to *resource changes* (match created, score finalised, player merged) — server-to-server, no persistent connection, signed payloads. WebSockets and Webhooks solve different problems: point-by-point during play vs. notifications when something changes.
  </Accordion>
...
<CardGroup cols={3}>
  <Card title="Live Match Endpoint" icon="bolt" href="/api-reference/match/show-match-point-by-point">
    Full reference for the point-by-point endpoint
  </Card>
  <Card title="Live Matches Endpoint" icon="arrows-rotate" href="/api-reference/live/list-live-matches">
