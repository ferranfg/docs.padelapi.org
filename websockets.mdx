...
The WebSocket event payload has the same structure as the `/api/matches/{match}/live` REST response — `id`, `self`, `channel`, `sets`, and `connections`.
...
4. **Detect match end** — When the match `status` changes to `finished` on the main match endpoint, unsubscribe from the channel
